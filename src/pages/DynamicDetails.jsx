...
const DynamicDetails = () => {
    ...
    useEffect(() => {
        console.log("asdfjasiofjaisfjaskfmadf");
        dispatch(AuthActions.dynamicProfile(uid));
        // if (uid) {
    ...
    console.log("asfasfdfsdfsfsfsfsdsfasf", reviewArr);
    ...
    let company_details = {
        ...
        data: [
            [(details?.roleName == 'Investor') ? "Investor Type" : "Project Type", (details?.roleName == 'Investor') ? ((investorType ? (investorType.map((itm) => itm + ", ")) : "")) : (details?.projectType)],
            (details?.roleName == 'Investor') ? ["stage focus", fundingStage] : (["Business Type", (details?.businessDesc !== 'Other') ? details?.businessDesc : details?.customBusinessDesc]),
            (details?.roleName == 'Investor') ? ["Geography focus", geography] : ([]),
            (details?.roleName == 'Investor') ? ["fund size / aum", "$111,000,000"] : ([]),
            (details?.roleName == 'Investor') ? ["investment range", "$100,000 - $2,500,000"] : ([]),
        ...
        <div className='dark:bg-darkBg dark:text-white'>
            <div className="grid grid-cols-1 xl:grid-cols-4 md:grid-cols-2 ">
                ...
                <div className='px-5 md:order-1 xl:order-1 order-1'>
                    <Company_profile_section company_links={company_details?.company_links} rating={company_details?.rating} votes={company_details.no_of_votes} logo_url={baseUrl + '/' + details?.clogo} />
                    <Company_people_connected connected_people={connected_people} people_type={"Connected Employees"} />
                </div>


                {/* Detail Section */}
                <Company_details_section reviews = {reviewArr} projectSummary={details?.descriptionFundSeeking} purpose={details?.descriptionUsageFund} roleName={details?.roleName} name={details?.cName} intro={details?.description} address={details?.address}
                    industry_focus={industryFocus} data={company_details.data} relevant_investments={company_details.relevant_investments} />
                {/* Button details */}
            ...
            <div className="grid lg:grid-cols-4 sm:grid-cols-6">
                <div className='sm:col-start-2 sm:col-end-6 lg:col-start-2 lg:col-end-4 col-span-4 px-10 sm:px-0'>
                    ...
                    <div className=" p-4 rounded-lg shadow-md">
                        ...
                        <p className="text-sm text-gray-600 mb-4 dark:text-white pt-5">#Abc Name</p>
